...
class RestrictedBoltzmannMachine():
    ...
    def __init__(self, ndim_visible, ndim_hidden, is_bottom=False, image_size=[28,28], is_top=False, n_labels=10, batch_size=10):
        ...
        self.delta_bias_v = np.zeros(shape=(self.ndim_visible))

        self.delta_weight_vh = np.zeros(shape=(self.ndim_visible, self.ndim_hidden))

        self.delta_bias_h = np.zeros(shape=(self.ndim_hidden))
    ...
    def cd1(self,visible_trainset, n_iterations=10000):
        ...
        for epoch in tqdm(range(n_iterations)):
            for iteration in tqdm(range(int(n_samples / self.batch_size))):
                # [TODO TASK 4.1] run k=1 alternating Gibbs sampling : v_0 -> h_0 ->  v_1 -> h_1.
                # you may need to use the inference functions 'get_h_given_v' and 'get_v_given_h'.
                # note that inference methods returns both probabilities and activations (samples from probablities) and you may have to decide when to use what.
                v_0 = visible_trainset[iteration * self.batch_size:iteration*self.batch_size + self.batch_size, :].copy()
                prob_h_0, h_0 = self.get_h_given_v(v_0)
                prob_v_1, v_1 = self.get_v_given_h(h_0)
                prob_h_1, h_1 = self.get_h_given_v(v_1)

                # [TODO TASK 4.1] update the parameters using function 'update_params'
                self.update_params(v_0, h_0, prob_v_1, h_1)
    ...
    def update_params(self,v_0,h_0,v_k,h_k):
        ...
        n_samples, _ = v_0.shape
        previous_delta_weight_vh = self.delta_weight_vh.copy()
        prev_delta_bias_v = self.delta_bias_v.copy()
        prev_delta_bias_h = self.delta_bias_h.copy()

        self.delta_bias_v = self.learning_rate * (np.sum(v_0, axis=0) - np.sum(v_k, axis=0)) + self.momentum * prev_delta_bias_v
        self.delta_weight_vh = self.learning_rate * ((1/n_samples) * (v_0.T @ h_0) - (1/n_samples) * v_k.T @ h_k) + self.momentum * previous_delta_weight_vh
        self.delta_bias_h = self.learning_rate * (np.sum(h_0, axis=0) - np.sum(h_k, axis=0)) + self.momentum * prev_delta_bias_h
    ...
    def get_v_given_h(self,hidden_minibatch):
        ...
        if self.is_top:
            ...
            sums = hidden_minibatch @ self.weight_vh.T

            prob_visible = sigmoid(sums[:, :-self.n_labels])
            prob_labels = softmax(sums[:, -self.n_labels:])

            probabilities = np.c_[prob_visible, prob_labels]
            samples = np.c_[sample_binary(prob_visible), sample_categorical(prob_labels)]
    ...
    def get_v_given_h_dir(self,hidden_minibatch):
        ...
        probabilities, samples = None, None
        ...
        if self.is_top:
            ...
            raise NotImplementedError('This should not be executed')
