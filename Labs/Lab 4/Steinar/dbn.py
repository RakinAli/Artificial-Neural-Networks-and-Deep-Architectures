...
class DeepBeliefNet():    
    ...
    def recognize(self,true_img,true_lbl):
        ...
        prob_hidden, hidden = self.rbm_stack['vis--hid'].get_h_given_v_dir(vis)
        prob_pen, pen = self.rbm_stack['hid--pen'].get_h_given_v_dir(prob_hidden)

        prob_pen_lbl = np.c_[prob_pen, lbl]

        for _ in range(self.n_gibbs_recog):
            prob_top, top = self.rbm_stack['pen+lbl--top'].get_h_given_v(prob_pen_lbl)
            prob_pen_lbl, pen_lbl = self.rbm_stack['pen+lbl--top'].get_v_given_h(prob_top)


        predicted_lbl = prob_pen_lbl[:, -self.n_labels:]
    ...
    def train_greedylayerwise(self, vis_trainset, lbl_trainset, n_iterations):
        ...
        try :
            ...
            """            
            self.rbm_stack["vis--hid"].untwine_weights()        
            prob_vis_hidden_layer_1, _ = self.rbm_stack['vis--hid'].get_h_given_v_dir(vis_trainset)

            self.rbm_stack['hid--pen'].cd1(prob_vis_hidden_layer_1, n_iterations=n_iterations)
            ...
            """
            self.rbm_stack["hid--pen"].untwine_weights()
            prob_hid_pen_hidden_layer, _ = self.rbm_stack['hid--pen'].get_h_given_v_dir(prob_vis_hidden_layer_1)

            self.rbm_stack['pen+lbl--top'].cd1(np.c_[prob_hid_pen_hidden_layer, lbl_trainset], n_iterations=n_iterations)
